...
USE universidad;
SELECT DISTINCT nombre, apellido1, apellido2 FROM persona WHERE tipo = 'alumno' ORDER BY apellido1 ASC, apellido2 ASC, nombre ASC;
SELECT DISTINCT nombre, apellido1, apellido2, telefono FROM persona WHERE tipo = 'alumno' AND telefono IS NULL;
SELECT DISTINCT nombre, apellido1, apellido2, fecha_nacimiento FROM persona WHERE tipo='alumno' AND fecha_nacimiento BETWEEN '1999-01-01' AND '1999-12-31';
...
SELECT asignatura.nombre, asignatura.creditos, asignatura.tipo FROM asignatura JOIN grado ON asignatura.id_grado = grado.id WHERE grado.id = 7 AND asignatura.curso = 3 AND asignatura.cuatrimestre = 1;
SELECT DISTINCT persona.apellido1, persona.apellido2, persona.nombre, departamento.nombre FROM persona JOIN profesor ON persona.id = profesor.id_profesor JOIN departamento ON departamento.id = profesor.id_departamento ORDER BY apellido1 DESC, apellido2 DESC, persona.nombre DESC;
...
SELECT dep.nombre, p.nombre, p.apellido1, p.apellido2 FROM persona p LEFT JOIN profesor pf ON p.id = pf.id_profesor LEFT JOIN departamento dep ON pf.id_departamento = dep.id WHERE p.tipo = 'profesor' ORDER BY dep.nombre DESC, p.apellido1 DESC, p.apellido2 DESC, p.nombre;
SELECT DISTINCT p.nombre, p.apellido1, p.apellido2 FROM persona p LEFT JOIN profesor pf ON p.id = pf.id_profesor LEFT JOIN departamento dep ON pf.id_departamento = dep.id WHERE pf.id_departamento IS NULL AND p.tipo='profesor';
...
SELECT DISTINCT CONCAT(p.nombre,' ',p.apellido1, ' ', p.apellido2) AS profesors_sense_asignatura FROM persona p LEFT JOIN asignatura ON asignatura.id_profesor = p.id WHERE p.tipo = 'profesor' AND asignatura.nombre IS NULL;
...
SELECT COUNT(DISTINCT p.id) FROM persona p WHERE tipo = 'alumno';
SELECT COUNT(DISTINCT p.id) FROM persona p WHERE p.tipo = 'alumno' AND p.fecha_nacimiento BETWEEN '1999-01-01' AND '1999-12-31';
